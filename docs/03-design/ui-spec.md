...
小程序使用**系统导航栏 + 自定义 header** 组合：

| 层级 | 元素 | 内容 | 高度 |
|------|------|------|------|
| 1 | 系统导航栏 | "过门不忘" 标题 | ≈ 44px |
| 2 | 自定义 Header | "今日" + 日期 | 40px |
| 3 | Filter Bar | 隐藏已确认 chip + 待确认计数 + **编辑按钮** | 36px |

**关键规格**：
- 页面内容需 `padding-top: 44px` 避开系统导航栏遮挡
- 编辑按钮位于 **filter-bar 右侧**，和"X 项待确认"并列

#### 布局参数

| 属性 | 值 |
|------|-----|
| 页面顶部安全间距 | `padding-top: 44px`（避开系统导航栏） |
| Header 高度 | `40px` |
| Filter Bar 高度 | `36px` |
| 日期文字 | `--font-size-sm`, `--color-text-soft` |
| "今日"标题 | `--font-size-xl`, `--color-text`, `font-weight: 700` |
| 编辑按钮 | `pill` 形状，`--radius-full`，边框 `--color-border` |
| 事项列表间距 | `--space-sm` |
| 页面边距 | `--space-md` 左右 |
| 底部导航高度 | `56px` + 安全区 |
...
Web 原型（`prototype.html`）无系统导航栏，布局结构：
- Header: "今日" + 日期
- Filter Bar: chip（左） + 待确认计数 + 编辑按钮（右）

小程序需对齐此结构，额外处理系统导航栏遮挡。
