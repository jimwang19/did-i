...
- **规格对照强制**: 实现任何 P0/P1 功能前，必须查 `docs/spec-implementation-matrix.md` 对照 Web 原型已有实现位置。PR 描述必须包含：(1) 规格章节引用（如 F3） (2) Web 原型已有实现位置（如 `prototype.html:1338`） (3) 本次小程序实现位置。三者不一致必须说明原因（参见 MEM-52 反省）
- **开发前五步门禁**（P0/P1 功能强制）：
  1. 打开 `docs/02-product/product-spec.md` → 找到对应 Feature 章节，确认**功能入口在哪个页面/组件**
  2. 打开 `docs/spec-implementation-matrix.md` → 找到对应行，阅读 Web 原型实现位置
  3. 打开 `prototype.html` 对应代码行，理解正确实现逻辑（不是凭直觉猜）
  4. 确认小程序实现与规格/原型一致后再提交
  5. 提交后在矩阵中回填"小程序实现位置"列，更新状态为 ✅
